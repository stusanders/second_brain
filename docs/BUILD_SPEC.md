...
Lint **flags and proposes; it does not auto-fix.** Output is a review queue the user works through, each item approved or dismissed individually. On the team tier, lint proposals that would alter existing content are never auto-applied (append-only still holds) — they surface as suggestions, and any resulting change goes through the normal push/review path.
